<template>
    <div>
     ...
     <modal :mdShow="isMdShow2" @close="isMdShow2=false">
         ...
         <div slot="btnGroup">
             <a href="javascript:;" class="btn btn--m btn--red" @click="isMdShow=false">好的</a>
         </div>
     </modal>
      ...
      methods: {
          ...
          setDefault(addressId){
            //   console.log('addressId: ' + addressId)
              let postData = qs.stringify({ addressId: addressId });
            //   console.log('postData: ' + postData)
              axios.post('/users/setDefault', postData)
              .then((response) => {
                let res = response.data;
                if(res.status == '0')
                {
          ...
          delAddress(){
              ...
              let postData = qs.stringify({ addressId: this.addressId });
              axios.post('/users/delAddress', postData)
              .then((response) => {
                  let res = response.data;
                  if(res.status == '0')
                  {
