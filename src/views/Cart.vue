        ...
        <div class="item-list-wrap">
          <div class="cart-item">
            ...
            <ul class="cart-item-list">
              <li v-for="item in cartList" :key='item.userId'>
...
import { currency } from './../util/currency'
    export default{
        ...
        methods: {
            ...
            // 删除购物车
            delCart(){
                let postData = qs.stringify({ productId: this.delItem.productId })
                axios.post('/users/cartDel', postData)
                .then((response) => {
                    let res = response.data;
                    if(res.status == '0')
                    {
            ...
            editCart(flag, item){
              ...
              axios.post('users/cartEdit', postData)
              .then((response) => {
                let res = response.data;
                let num = 0;
                if(flag == 'add')
            ...
            toggleCheckAll(){
              ...
              let postData = qs.stringify({ checkAll: flag });
              // console.log('postData: ' + postData);
              axios.post('/users/editCheckAll', postData)
              .then((response) => {
                let res = response.data;
                if(res.status == '0')
                {
